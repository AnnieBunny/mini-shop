...
const Header = ({ cart, setCart, setExpanded }) => {
    ...
    const [myOrdersModalShow, setMyOrdersModalShow] = useState(false);

    const handleShowOrdersList = () => {
        if (token) {
            headerService.getUserOrders(token)
                .then(data => {
                    setOrders(data)
                    setMyOrdersModalShow(true);
                }
    ...
    const handlePayment = (cart) => {
        headerService.createCheckoutSession(cart,token)
    ...
    return (
        <>
            <Navbar bg="dark" variant="dark" expand="lg" style={{ position: 'relative' }}>
                <Container>
                    ...
                    <Nav className="ms-auto">
                        ...
                        {token ? (
                            <>
                                <Button style={{ color: 'black', background: 'lightgreen', marginLeft: '5px' }} onClick={handleShowOrdersList}>My orders</Button>
            ...
            <MyOrdersModal
                orders={orders}
                show={myOrdersModalShow}
                close={() => setMyOrdersModalShow(false)}
            />
        </>
    );
};
